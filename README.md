...
- `@log` (usage: `@log var_name = val`): variables annotated by this macro will be logged (Actually this stands for `__LOGGER_DICT__[var_name] = val`).
    - `@log_only`: the same as `@log` but it will be activated only when logging variables. It is not activated when solving DEProblem.
    - This macro is highly inspired by [SimulationLogs.jl](https://github.com/jonniedie/SimulationLogs.jl).
- `@nested_log` (usage: `@nested_log env_name ODEFunction_call`): this macro will save all variables logged in `ODEFunction_call` as `__LOGGER_DICT__[env_name]`. `ODEFunction_call` should be annotated by `@Loggable`.
